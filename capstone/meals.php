  ...
  background-color: #ffff99;
	...
	<body onload="startTime(),ini()">
		...
		<div id="productcontainer" style="position:relative;">
			...
			<div id="editcontent">
				<span id="addtitle">MEALS AVAILABLE</span></br></br>
...
$nume = $dbo->query("select count(id) from product where deleted = 0 and status='AVAILABLE'")->fetchColumn();


echo "<table id='sample' style='margin-left:0;margin-top:2%;'>";
echo  "<tr style='text-align:center;'><th style='text-align:center;'>IMAGE</th><th style='text-align:center;'>NAME</th><th style='text-align:center;'>DESCRIPTION</th><th style='text-align:center;'>CATEGORY</th><th style='text-align:center;'>PRICE</th>
<th style='text-align:center;'>STATUS</th></tr>";


$query=" SELECT * FROM product where deleted=0 and status='AVAILABLE' order by name limit $eu, $limit ";
...
echo "<tr class='r$m'><td><img src='images/products/$row[images]' style='width:80px;height:80px;'></td><td>$row[name]</td><td>$row[description]</td>
<td>$row[category]</td><td>$row[price]</td><td>$row[status]</td></tr>";
}
if($nume > $limit ){ 
