...
function getDay($day)
	...
	function Footer()
	{
		...
		function LoadData()
		{
			...
			if(isset($_GET['id'])){
			$id = $_GET['id'];
			$result=mysql_query("SELECT * from transaction where id='$id' and date BETWEEN '{$begin}' AND '{$end}' order by id desc");
			...
			$result=mysql_query("SELECT * from transaction where date BETWEEN '{$begin}' AND '{$end}' order by id desc");
			while($row=mysql_fetch_row($result)) 
			{ 
				$data[] = $row;
			}
