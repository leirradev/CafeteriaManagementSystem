...
header ("Location: adminlogin.php");
		...
		function Header()
    {
		...
		//
		$this->Cell(-35,40,'List of Meals',0,0,'C');
		//Line break
		$this->Ln(30);
	}
	...
	function Footer()
	{
		...
		//Page number
		$this->Cell(0,10,'Page '.$this->PageNo().' Meals Report',0,0,'C');
	}
	
	
		function LoadData()
		{
			$result=mysql_query("SELECT * from user where deleted = 0 order by _username");
		...
		{
			$name = $_GET['category'];
			$result1=mysql_query("SELECT * from product where category LIKE '%{$name}%' and deleted = 0 order by name");
		...
		function FancyTable($header,$data)
		{
			...
			foreach($data as $row)
			{
				...
				$this->Cell($w[3],6,"Php ".$row[5],'LR',0,'C',$fill);
...
$pdf=new PDF();
//Column titles
//$header=array('Username','Contact Number','Parents Contact Number','Load','Status');
