...
header ("Location: adminlogin.php");
		...
		function Header()
    {
		...
		$this->Cell(80);
		//Title
		$this->Cell(150,10,'Prepaid Cafeteria Monthly Report',0,0,'C');
	...
	function Footer()
	{
		...
		//Page number
		$this->Cell(0,10,'Page '.$this->PageNo().'Monthly-Report',0,0,'C');
	}
		...
		function LoadData()
		{
			...
			if(isset($_GET['id'])){
			$id = $_GET['id'];
			$result=mysql_query("SELECT * from transaction where id='$id' and date LIKE '{$date}%' order by id desc");
			...
			$result=mysql_query("SELECT * from transaction where date LIKE '{$date}%' order by id desc");
			while($row=mysql_fetch_row($result)) 
			{ 
				$data[] = $row;
			}
